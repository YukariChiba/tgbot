...
def load():
    ...
    print("Breed Inline Plugin Loaded!")
...
def makeBreed(user1, user2):
    ...
    base.save(os.getenv("MODULE_BREED_INLINE_TMP"), "WEBP")
...
def run(querybody, context):
    ...
    if user2 == None or user1 == None:
        return None
    im = makeBreed(user1id, user2id)
    file_upload = upload(os.getenv("MODULE_BREED_INLINE_TMP"), context.bot)
