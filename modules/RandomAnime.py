...
def checkarg(arg):
    if len(arg) == 0:
        return random.randint(1000, 49999)
...
def run(update: Update, context: CallbackContext) -> None:
    ...
    if seed == -1:
        update.message.reply_text(
            "*Get a random anime from thisanimedoesnotexist.ai .*\nUsage: `/TADNE [seed]`.", parse_mode='Markdown')
    else:
        seed = str(seed)
        update.message.reply_photo(
            "https://thisanimedoesnotexist.ai/results/psi-1.0/seed{}.png".format(seed), caption="Random Seed: {}".format(seed), parse_mode='Markdown')
